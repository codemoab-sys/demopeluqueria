...
<div class="card-modern" style="max-width:560px;margin:0 auto;">
    ...
    <form method="POST" action="{{ route('categorias-productos.store') }}" class="card-body">
        @csrf
        <div class="mb-3"><label class="form-label">Nombre *</label><input type="text" name="nombre" class="form-control" required></div>
        <div class="mb-3"><label class="form-label">Color</label><input type="color" name="color" class="form-control form-control-color" value="#8b5cf6"></div>
        ...
        <div class="d-flex justify-content-between"><a href="{{ route('categorias-productos.index') }}" class="btn btn-soft">Cancelar</a><button class="btn btn-primary">Crear</button></div>
    </form>
</div>
@endsection
